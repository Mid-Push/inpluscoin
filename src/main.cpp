...
#if defined(NDEBUG)
# error "Inpluscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x77daf22b642d18fa2a0ad713b2536bc254f97377f613aa5760e41838c85fd40d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Inpluscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Inpluscoin Signed Message:\n";
...
{
    // Inpluscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Inpluscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10*30; // Inpluscoin: 3.5 days
static const int64 nTargetSpacing = 1 * 20; // Inpluscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Inpluscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Inpluscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x77daf22b642d18fa2a0ad713b2536bc254f97377f613aa5760e41838c85fd40d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "INPLUSCOIN IN 2016!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xafd536c3ad39e87fbb599c17f50dfcf679b320c88cd7e267f505a1d06ce77200"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Inpluscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InpluscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InpluscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InpluscoinMiner(CWallet *pwallet)
{
    printf("InpluscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("inpluscoin-miner");
    ...
    try { loop {
        ...
        printf("Running InpluscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InpluscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InpluscoinMiner, pwallet));
}
